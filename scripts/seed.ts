...
import { Vet } from "../src/lib/models/Vet"; // Added Vet model
...
async function seedDatabase() {
  try {
    ...
    console.log("🧹 Skipping clearing of existing data to protect current data...");
    ...
    // 2. SEED VET PROFILE
    // ----------------------------------------------------------------------
    const vetProfileCount = await Vet.countDocuments();
    if (vetProfileCount === 0 && vetUser) {
      console.log("🌱 Seeding Vet Profile...");
      await Vet.create({
        user: vetUser._id,
        specialization: "General & Exotic Animals",
        bio: "Dr. Sarah has over 10 years of experience treating cats, dogs, and exotic birds. Dedicated to providing compassionate care.",
        experience: 10,
        consultationFee: 50,
      ...
      console.log("⏭️ Vet profiles collection is not empty, skipping...");
    }

    // ----------------------------------------------------------------------
    // 3. SEED CATEGORIES
    // ----------------------------------------------------------------------
    let createdCategories: Record<string, any> = {};
    ...
    if (catCount === 0) {
      ...
      console.log("⏭️ Categories collection is not empty, skipping...");
    }

    // ----------------------------------------------------------------------
    // 4. SEED PRODUCTS
    // ----------------------------------------------------------------------
    let productsDb: Record<string, any> = {};
    const prodCount = await Product.countDocuments();

    if (prodCount === 0) {
      console.log("🌱 Seeding Products...");

      // Fetch categories if they were skipped above so we can link products to them
      if (Object.keys(createdCategories).length === 0) {
        const allCats = await Category.find();
        for (const cat of allCats) createdCategories[cat.slug] = cat;
      }
      ...
      console.log("⏭️ Products collection is not empty, skipping...");
      // Fetch existing products to use them for Carts/Orders if they are empty
      const existingProducts = await Product.find();
      for (const p of existingProducts) productsDb[p.name] = p;
    }

    // ----------------------------------------------------------------------
    // 5. SEED FAVORITES (WISHLIST)
    // ----------------------------------------------------------------------
    // Only attempt to seed wishlist if the customer exists and wishlist is empty
    if (customer && customer.wishlist && customer.wishlist.length === 0 && Object.keys(productsDb).length > 0) {
      ...
      ].filter(Boolean); // filter out undefined if products were missing
      await customer.save();
    } else {
      console.log("⏭️ User already has a wishlist, skipping...");
    }
    ...
    if (cartCount === 0 && customer && Object.keys(productsDb).length > 0) {
      console.log("🌱 Seeding Cart...");
      if (productsDb['Golden Buddy'] && productsDb['Fluffy Persian'] && productsDb['Snowball Bunny']) {
        await Cart.create({
          user: customer._id,
          items: [
      ...
      console.log("⏭️ Cart collection is not empty, skipping...");
    }

    // ----------------------------------------------------------------------
    // 7. SEED RECENT ORDERS
    // ----------------------------------------------------------------------
    const orderCount = await Order.countDocuments();
    if (orderCount === 0 && customer && Object.keys(productsDb).length > 0) {
      console.log("🌱 Seeding Recent Orders...");

      if (productsDb['Golden Retriever'] && productsDb['Persian Cat'] && productsDb['Macaw Parrot']) {
        await Order.create({
          user: customer._id,
          items: [
            {
              ...
              quantity: 1,
              image: String(productsDb['Golden Retriever'].images || productsDb['Golden Retriever'].images)
            }
          ],
          total: 1299,
          status: "delivered",
          ...
          items: [
            {
              ...
              quantity: 1,
              image: String(productsDb['Persian Cat'].images || productsDb['Persian Cat'].images)
            }
          ],
          total: 899,
          status: "shipped",
          ...
          items: [
            {
              ...
              quantity: 1,
              image: String(productsDb['Macaw Parrot'].images || productsDb['Macaw Parrot'].images)
            }
          ],
          total: 1499,
          status: "pending",
      ...
      console.log("⏭️ Orders collection is not empty, skipping...");
    }

    console.log("🎉 Database seeded successfully without affecting existing data!");
    process.exit(0);
  }
  catch (error) {
