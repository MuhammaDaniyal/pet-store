...
interface AppConfig {
  ...
  scrollSpeed?: number;
  scrollEase?: number;
  onItemClick?: (item: { image: string; text: string }) => void; // ADDED CLICK PROP
}
...
class App {
  ...
  isDragging: boolean = false; // ADDED DRAG TRACKER
  start: number = 0;
  ...
  constructor(container: HTMLElement, config: AppConfig) {
    ...
    this.onItemClick = config.onItemClick; // BIND CLICK PROP
    ...
    this.onResize();
    this.createGeometry();
    this.createMedias(config.items, config.bend, config.textColor || '#ffffff', config.borderRadius || 0, config.font || 'bold 30px Figtree');
    this.update();
  ...
  onTouchDown(e: MouseEvent | TouchEvent) {
    this.isDown = true;
    this.isDragging = false; // RESET DRAG ON CLICK
  ...
  onTouchMove(e: MouseEvent | TouchEvent) {
    ...
    // IF MOVED MORE THAN 5px, IT IS A DRAG, NOT A CLICK
  ...
  onTouchUp() {
    ...
    // TRIGGER CLICK IF NO DRAGGING OCCURRED
    if (!this.isDragging && this.onItemClick && this.medias.length > 0) {
      const width = this.medias[0].width;
      const itemIndex = Math.round(Math.abs(this.scroll.target) / width);
      const activeItem = this.mediasImages[itemIndex % this.mediasImages.length];
  ...
  onCheck() {
    if (!this.medias || !this.medias) return;
    const width = this.medias[0].width;
    const itemIndex = Math.round(Math.abs(this.scroll.target) / width);
...
export interface CircularGalleryProps {
  ...
  scrollSpeed?: number;
  scrollEase?: number;
  onItemClick?: (item: { image: string; text: string }) => void; // EXPORTED PROP
}
