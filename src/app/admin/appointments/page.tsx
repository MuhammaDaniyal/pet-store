...
export default async function AdminAppointmentsPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8 lg:py-12">
      <div className="mb-8">
        <h1 className="text-[clamp(1.8rem,3vw,2.6rem)] font-semibold tracking-[-0.04em] text-primary">Appointments Overview</h1>
        <p className="mt-3 text-base text-secondary">View all appointments across the platform.</p>
      </div>
      ...
      <div className="overflow-x-auto rounded-[28px] border border-border bg-surface shadow-[0_16px_40px_rgba(26,83,92,0.05)]">
        <table className="w-full text-left whitespace-nowrap">
          <thead className="border-b border-border bg-background/50">
            <tr>
              <th className="px-5 py-4 text-xs font-semibold uppercase tracking-wider text-muted">Date & Time</th>
              <th className="px-5 py-4 text-xs font-semibold uppercase tracking-wider text-muted">Customer</th>
              <th className="px-5 py-4 text-xs font-semibold uppercase tracking-wider text-muted">Vet</th>
              <th className="px-5 py-4 text-xs font-semibold uppercase tracking-wider text-muted">Pet</th>
              <th className="px-5 py-4 text-xs font-semibold uppercase tracking-wider text-muted">Type</th>
            ...
            {appointments.map((appt: any) => {
              ...
              return (
                <tr key={appt._id.toString()} className="border-b border-border/50 hover:bg-surface/50 transition-colors">
                  ...
                  <td className="px-5 py-4 text-sm text-secondary">{appt.user?.name || appt.user?.email || "Unknown"}</td>
                  <td className="px-5 py-4 text-sm text-secondary">{appt.vet?.name || appt.vet?.email || "Unknown"}</td>
                  ...
                  <td className="px-5 py-4 text-sm text-secondary capitalize">{appt.type}</td>
