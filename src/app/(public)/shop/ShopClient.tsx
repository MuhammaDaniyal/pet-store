...
import { Heart, AlertTriangle, LayoutGrid, ListTree } from "lucide-react";
  ...
  const ProductCard = ({ p }: { p: ProductItem }) => {
    ...
    return (
      <Link href={`/shop/${p._id}`} className="block h-full">
        {/* HOVER ANIMATION CLASSES APPLIED HERE */}
        <article className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-border/50 bg-card text-card-foreground transition-all duration-300 hover:-translate-y-1 hover:shadow-xl dark:hover:shadow-primary/10">
          ...
          <div className="relative h-56 w-full bg-muted/50">
            ...
            {/* Heart Icon */}
            <button 
              className="absolute right-3 top-3 flex h-8 w-8 items-center justify-center rounded-full bg-background/90 shadow-sm transition-colors hover:bg-red-50 hover:text-red-500 dark:bg-background/50 dark:hover:bg-background/90"
              onClick={(e) => e.preventDefault()} // Prevent clicking heart from navigating to product page
            >
